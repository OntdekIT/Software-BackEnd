...
public class NeighbourhoodUnitTest {
    ...
    public void init() {
        ...
        measurementList.add(new Measurement(1, now, 51.578000f, 4.992980f, 22.0f, 50.0f, 12.5f));
        measurementList.get(0).setIs_public(true);
        measurementList.add(new Measurement(2, now, 51.578000f, 40.992980f, 23.0f, 50.0f, 10.2f));
        ...
        measurementList.add(new Measurement(3, now, 51.5869f, 4.999210f, 23.0f, 50.0f, 15.0f));
        measurementList.get(2).setIs_public(true);

        measurementList.add(new Measurement(4, now, 51.5869f, 4.999210f, 22.0f, 50.0f, 11.3f));
        measurementList.get(3).setIs_public(false);
    }

    @Test
    public void testNeighbourhoodConstructor() {
        // Arrange
        long expectedId = 10L;
        String expectedName = "TestDistrict";

        // Act
        Neighbourhood neighbourhood = new Neighbourhood(expectedId, expectedName);

        // Assert
        assertEquals(expectedId, neighbourhood.getId());
        assertEquals(expectedName, neighbourhood.getName());
        assertEquals(null, neighbourhood.getCoordinates()); // Because only id and name were set
    }
